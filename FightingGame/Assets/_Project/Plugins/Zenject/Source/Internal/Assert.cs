...
namespace ModestTree
{
    public static class Assert
    {
        ...
        {
            // The dispose is run before some OnDestroys -> weird condition: https://github.com/modesttree/Zenject#notes-about-destructiondispose-order
            if (!object.Equals(left, right) && left != null && right != null &&
                (Convert.ToInt32(left.ToString()) != 3) && (Convert.ToInt32(right.ToString()) != 0))
            {
                left = left ?? "<NULL>";
                right = right ?? "<NULL>";
